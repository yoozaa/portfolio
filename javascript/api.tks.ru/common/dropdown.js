...
class Dropdown extends Component {
  constructor (props) {
    super(props)
    this.state = {
      options: typeof props.placeholder === 'undefined' ? []: [...props.options],
      value: props.value,
      selected: this.parseValue(props.value, props.options) || {
        label: typeof props.placeholder === 'undefined' ? DEFAULT_PLACEHOLDER_STRING : props.placeholder,
        value: ''
      },
      isOpen: typeof props.isOpen === 'undefined' ? false : props.isOpen
    }
    this.mounted = true
  ...
  setSelected(value) {
      this.componentWillReceiveProps({value: value, options: this.state.options})
  }
  ...
  componentWillReceiveProps (newProps) {
    if (newProps.value) {
      ...
      if (selected !== this.state.selected) {
        this.setState({selected: selected, value: newProps.value})
      }
    } else {
  ...
  parseValue (value, options) {
    ...
    if (typeof value === 'string') {
      for (var i = 0, num = options.length; i < num; i++) {
        ...
        } else if (typeof options[i].key !== 'undefined' && options[i].key === value) {
          option = options[i]
        }
      }
    }
  ...
  buildMenu () {
    ...
    let ops = options.map((option) => {
      if (option.type === 'group') {
        ...
        return (
          <div className={`${baseClassName}-group`} key={option.key} role='listbox' tabIndex='-1'>
            {groupTitle}
            {_options}
          </div>
        )
